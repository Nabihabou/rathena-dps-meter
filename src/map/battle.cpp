...
int64 battle_calc_damage(struct block_list *src, struct block_list *bl, struct Damage *d, int64 damage, uint16 skill_id, uint16 skill_lv)
{
	...
	std::time_t timestamp = std::chrono::system_clock::to_time_t(now);			  // Convert the time point to the duration since epoch
	// Send the damage to the player's dps meter
	if (src->type == BL_PC)
	{
		// std::cout << skill_get_elemental_type(skill_id, skill_lv) << std::endl;
		sd = BL_CAST(BL_PC, src);
		meter_send_damage(sd->status.account_id, skill_id, sd->status.char_id, bl->id, damage, src->m, false, timestamp);
	}
...
#endif
			if (sc && sc->getSCE(SC_WATK_ELEMENT))
...
static void battle_calc_attack_left_right_hands(struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if (sd)
	{
		...
		else if (is_attack_right_handed(src, skill_id) && is_attack_left_handed(src, skill_id) && sd->status.weapon != W_KATAR)
		{ // Dual-wield
#else
		}
